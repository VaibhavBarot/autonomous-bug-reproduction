...
export class OutputFormatter {
    ...
    static formatConsoleOutput(suggestion: FixSuggestion): string {
        ...
        // Fixes
        output += chalk.cyan.bold('🛠️  SUGGESTED FIXES\n');
        ...
        // Footer
        output += chalk.blue.bold('='.repeat(80) + '\n');
        output += chalk.gray('💡 Tip: Review the diffs carefully before applying them to your code.\n');
        output += chalk.blue.bold('='.repeat(80) + '\n');
...
`;
    ...
    static formatSummary(suggestion: FixSuggestion, verbose: boolean = false): string {
        ...
        summary += chalk.white(`📊 Summary:\n`);
        summary += chalk.gray(`   • Affected files: ${suggestion.affectedFiles.length}\n`);
        summary += chalk.gray(`   • Total fixes: ${suggestion.fixes.length}\n\n`);

        if (verbose) {
            summary += chalk.white(`📁 Files to modify:\n`);
            suggestion.affectedFiles.forEach(file => {
                summary += chalk.cyan(`   • ${file}\n`);
            });
            summary += '\n';
        }
