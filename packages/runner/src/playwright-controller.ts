...
export class PlaywrightController {
  ...
  async initialize(headless: boolean = false, stagehandApiKey?: string, stagehandModelProvider?: string): Promise<void> {
    ...
    if (stagehandApiKey) {
      try {
        console.error('[PlaywrightController] Attempting to initialize Stagehand with API key...');
        ...
        console.error('[PlaywrightController] Stagehand initialized, CDP endpoint:', cdpEndpoint);
        ...
        console.error('[PlaywrightController] Got browser and page from Stagehand');
        
        // Get or create the context
        const contexts = this.browser.contexts();
        if (contexts.length > 0) {
          this.context = contexts[0];
          console.error('[PlaywrightController] Using existing context from Stagehand browser');
        } else {
          // Create a new context with our settings
          console.error('[PlaywrightController] Creating new context with video recording...');
          ...
          console.error('[PlaywrightController] Created new page in new context');
        }
        
        console.error('[PlaywrightController] ✅ Successfully connected to Stagehand-managed browser via CDP');
      } catch (error: any) {
